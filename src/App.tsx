...
const sketch = (p: p5) => {
    ...
    let stitches = [FIRST_STITCH];
    let hovering: Stitch | null = null;
    let dragging: DragState | null = null;
    let detailedView = false;
    let nextParent = FIRST_STITCH;
    ...
    const deleteStitch = (stitch: Stitch) => {
        ...
        if (nextParent === stitch) nextParent = stitch.parent || FIRST_STITCH;
        // Remove the stitch
        ...
        switch (stitch.type) {
            ...
            case StitchType.Chain:
                return stitch.parent === null
                    ? headAnchor(stitch)
                    : p5.Vector.add(headAnchor(stitch.parent), stitch.pos).div(
                          2
                      );
            case StitchType.SingleCrochet:
            case StitchType.HalfDoubleCrochet:
            ...
            case StitchType.DoubleTrebleCrochet:
                return stitch.base === null
                    ? headAnchor(stitch)
                    : p5.Vector.add(attachAnchor(stitch.base), stitch.pos).div(
                          2
                      );
        }
    }
    ...
    function anchorForMode(stitch: Stitch): p5.Vector {
        switch (mode) {
            ...
            case EditingMode.Deleting:
                return middleAnchor(stitch);
            case EditingMode.Rebasing:
                return dragging ? attachAnchor(stitch) : middleAnchor(stitch);
            case EditingMode.Adding:
    ...
    function drawSingleCrochetSitch(stitch: Stitch) {
        if (stitch.base === null) {
            console.warn("Tried to draw single crochet stitch with no base");
            return;
        }
    ...
    function drawLongStitchWithHashes(stitch: Stitch, nHashes: number) {
        if (stitch.base === null) {
            console.warn(`Tried to draw ${stitch.type} stitch with no base`);
            return;
        }
        if (stitch.parent === null) {
            console.warn(
    ...
    p.draw = () => {
        ...
        for (const stitch of stitches) {
            ...
            p.noFill();
            if (canDrag(stitch)) {
                if (mode === EditingMode.Moving && groupSelection.has(stitch)) {
